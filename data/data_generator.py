...
def create_tick_data(
        ...
        start_price: float = 1000.00, mu: float = .0, var: float = 1.0,
        avg_freq_ms: float = 10., avg_size: float = 1000, lot_size: float = 10.):

    price = np.random.normal(mu, var, n_samples)
    price[0] = start_price
    price = price.cumsum()

    ts = np.random.exponential(avg_freq_ms, n_samples)
    ts = pd.to_timedelta(ts.cumsum(), unit='ms')
    ts = pd.Timestamp(2023, 1, 2) + ts

    volume = np.random.exponential(avg_size, n_samples)
    volume = np.round(volume / lot_size) * lot_size
    bidask = np.random.uniform(0, 1, n_samples)
    bidask = bidask < 0.5
...
if __name__ == '__main__':
    create_tick_data(n_samples=1000)
